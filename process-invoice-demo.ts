/**
 * Demo script for invoice processing using the structured data format
 */
...
// Match against POs
const mockPoIds = [43121, 43122, 43123];
let matchedPO = null;
let matchScore = 0;
const issues: string[] = [];

// Simple matching based on PO number
if (mockPoIds.length > 0) {
    const matchingPOId = mockPoIds.find(id => id.toString() === po_number);
    
    if (matchingPOId) {
        matchedPO = matchingPOId;
        matchScore = 1.0; // Perfect match
        console.log(`\nPerfect match found with PO: ${matchedPO}`);
    } else {
        // Fallback to first PO ID
        matchedPO = mockPoIds[0];
        ...
        console.log(`\nNo exact match found. Using ${matchedPO} with match score ${matchScore}`);
        console.log(`Issues: ${issues.join(", ")}`);
    }
}

// Export the invoice data for use in other modules
