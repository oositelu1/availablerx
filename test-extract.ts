...
/**
 * Test script to directly extract product details from the EPCIS XML
 */
async function testExtractProductDetails() {
  try {
    const filePath = './attached_assets/shipment_5d42d452-498b-419c-8346-087dc7a64828.epcis.xml';
    ...
    // Get the EPCISDocument root (with or without namespace)
          ...
          if (!vocabularies) {
            ...
            for (let i = 0; i < vocabularies.length; i++) {
              ...
              if (vocabulary.$ && vocabulary.$.type) {
                ...
                if (vocabulary.$.type.value === 'urn:epcglobal:epcis:vtype:EPCClass') {
