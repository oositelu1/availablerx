...
import { Timeline, TimelineContent, TimelineConnector, TimelineItem, TimelineSeparator } from '@/components/ui/timeline';
import { Separator } from "@/components/ui/separator";
import { History, Building, ArrowRight, Truck, Calendar, FileText } from "lucide-react";
...
export function EnhancedTransactionHistory({ history }: EnhancedTransactionHistoryProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Timeline className="md:px-2">
        {history.map((record, index) => (
          <TimelineItem key={`history-${index}`} className="pb-8">
            <TimelineSeparator>
              <div className="h-10 w-10 rounded-full border bg-background flex items-center justify-center">
                <History className="h-5 w-5 text-primary" />
              </div>
              {index < history.length - 1 && <TimelineConnector className="h-8" />}
            </TimelineSeparator>
            
            <TimelineContent className="pt-1 pb-8">
